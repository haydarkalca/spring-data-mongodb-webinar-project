...
class TweetDAOImpl implements TweetDAO {
    ...
    @Autowired
    public TweetDAOImpl(final MongoOperations mongos) {
        this.mongos = mongos;
    }

    public Tweet findTweetBy(String id) {
        return mongos.findOne(Query.query(where("_id").is(id)), Tweet.class);
    }
    ...
    public long countTweetsByVerifiedUsers() {
        Criteria criteria = where("user.verified").is(true);
        return countTweets(criteria);
    }

    public long countTweetsWrittenIn(String language) {
        Criteria criteria = where("lang").is(language);
        return countTweets(criteria);
    }

    public long countTweetsWrittenIn(String... languages) {
        Criteria criteria = where("lang").in(languages);
        return countTweets(criteria);
    }

    public long countTweetsBy(String twitterUser) {
        Criteria criteria = where("user.name").is(twitterUser);
        return countTweets(criteria);
    }

    public long countTweetsIncluding(String hashtag) {
        Criteria criteria = where("entities.hashtags.text").regex(hashtag, "i");
        return countTweets(criteria);
    }
    ...
    public long countTweetsByRetweetsGreaterThan(int favoriteCount) {
        Criteria criteria = where("retweet_count").gte(favoriteCount);
        return countTweets(criteria);
    }
    ...
    public long countUsersByFollowersGreaterThan(int followersCount) {
        Aggregation agg = Aggregation.newAggregation(
                match(where("user.followers_count").gte(followersCount)),
